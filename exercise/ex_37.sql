    ...
    INSERT INTO game (
      game_id, game_date, stadium, opponent, 
      own_score, opponent_score
    ) 
    VALUES 
      (
        1, '2021-02-20', 'Deutsche Bank Park (Frankfurt)', 
...
LEFT JOIN player as t3 ON t1.player_id = t3.player_id; -- new view
...
CREATE VIEW forward_player_v AS 
SELECT t1.game_id,
		t3.first_name,
		t3.last_name,
		t3.age,
...
WHERE t3.position == 'forward'; -- second view


SELECT * 
FROM forward_player_v
WHERE number == 9;

DROP VIEW IF EXISTS forward_player_v;


CREATE TRIGGER validate_position BEFORE INSERT
ON player
BEGIN 	
	SELECT CASE
			WHEN NEW.position NOT IN ('goalkeeper', 'defender', 'midfielder', 'forward')
			THEN RAISE (ABORT, 'Invalid position')
			END;
END; -- new TRIGGER

INSERT INTO player (player_id, first_name, last_name, age, number, position)
VALUES (1, 'Robert', 'Lewandowski', 33, 9, 'center forward'); -- ERROR INVALID POSITION !